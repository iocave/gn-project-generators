...
class ProjectGenerator:
    ...
    def _write_project(self, target):
        ...
        build_target = ["Target", {"Name": "Build"},
                        ["Exec", {"Command": "call directory-lock.exe . ninja.exe " +  build_target_name,
                        "WorkingDirectory": "$(OutDir)"}]]
        pr.append(build_target)

        clean_target = ["Target", {"Name": "Clean"},
                        ["Exec", {"Command": "call directory-lock.exe . ninja.exe -t clean " +  build_target_name,
                        "WorkingDirectory": "$(OutDir)"}]]
        pr.append(clean_target)
        
        compile_target = ["Target", {"Name": "ClCompile", "DependsOnTargets": "SelectClCompile"},
                        # SelectCLCompile leaves precompiled header creation in, but can ignore it - ninja will take care of that
                        ["Exec", {"Condition": "'%(ClCompile.PrecompiledHeader)' != 'Create' and '%(ClCompile.ExcludedFromBuild)'!='true' and '%(ClCompile.CompilerIteration)' == '' and @(ClCompile) != ''",
                                  "Command": "call directory-lock.exe . ninja.exe %(ClCompile.OutputFile)",
                                  "WorkingDirectory": "$(OutDir)"}]]
